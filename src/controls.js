...
export function updatePlaneAxis(x, y, z, planePosition, camera) {
  //ease in speed
  jawVelocity *= 0.95;
  if (controls.shift) pitchVelocity *= 0.95;
  else pitchVelocity *= 0.5;
  ...
  if (controls["w"]) pitchVelocity -= 0.0025;

  if (controls["s"]) pitchVelocity += 0.0025;
